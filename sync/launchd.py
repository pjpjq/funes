...
def install(home=None, *, config=None, python=None):
    ...
    native_content=plistlib.dumps(render_native_plist(config, python, native_env))
    snapshots=_snapshot_files((native_script, warm_helper, main_path, native_path))
...
def status(home=None, *, config=None):
    config=_config(home, config)
    return {label:{"path":str(path), "installed":path.exists(), "loaded":_launchctl_status(label)} for path, label in _owned_plists(config)}


def uninstall(home=None, *, config=None):
    config=_config(home, config)
    paths=plist_paths(config.home)
    _validate_plists(_owned_plists(config))
    stop(config=config)
    for path in paths:
        if path.exists():
            path.unlink()
    for path in helper_paths(config):
        if path.exists():
            path.unlink()
    return paths
